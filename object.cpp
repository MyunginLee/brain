...
public:
  ...
  void init() override
  {
    ...
    createInternalTriggerParameter("carMul", 4.15, 0.0, 20.0);
    createInternalTriggerParameter("modMul", 0.30, 0.0, 20.0);
    ...
    createInternalTriggerParameter("table", 3, 0, 8);
...
public:
  ...
  virtual void onInit() override
  {
    ...
    ascene->getBounds(scene_min_half, scene_max_half);
    scene_center_half = (scene_min_half + scene_max_half) / 2.f;
    // ascene->print();
    mMesh_half.resize(ascene->meshes());
    for (int i = 0; i < ascene->meshes(); i += 1)
    {
      ascene->mesh(i, mMesh_half[i]);
    }
